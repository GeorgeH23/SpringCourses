package com.george.recipeapp.bootstrap;

import com.george.recipeapp.domain.*;
...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent contextRefreshedEvent) {
        List<Recipe> recipeList = recipeReactiveRepository.findAll().collectList().toProcessor().block();
        if (recipeList != null && recipeList.isEmpty()) {

            categoryReactiveRepository.deleteAll()
                    .concatWith(unitOfMeasureReactiveRepository.deleteAll())
                    .then()
                    .block();
    ...
    private Mono<Void> loadCategories() {
        ...
        categoryReactiveRepository.save(cat2);
        categories.add(cat2);
        ...
        return categoryReactiveRepository.saveAll(categories).then();
    }
    ...
    private Mono<Void> loadUnitOfMeasures() {
        ...
        return unitOfMeasureReactiveRepository.saveAll(units).then();
    }
    ...
    private Flux<Recipe> saveRecipes() {
        return Flux.concat(insertGuacamoleRecipe(), createRecipeTacos())
                .collectList()
                .log()
                .flatMapMany(recipeReactiveRepository::saveAll)
    ...
    private Mono<Recipe> insertGuacamoleRecipe() {
        ...
        categories.add(categoryReactiveRepository.findByDescription("American").block());
        categories.add(categoryReactiveRepository.findByDescription("Mexican").block());
    ...
    private Recipe insertGuacamoleIngredients(Recipe recipe, Map<String, UnitOfMeasure> units) {
        ...
        recipe.addIngredient(new Ingredient("minced red onion or thinly sliced green onion", new BigDecimal(2), tablespoonUom));
        recipe.addIngredient(new Ingredient("serrano chiles, stems and seeds removed, minced", new BigDecimal(2), numberUom));
        ...
        recipe.addIngredient(new Ingredient("ripe tomato, seeds and pulp removed, chopped", new BigDecimal(".5"), numberUom));
    ...
    private Mono<Recipe> createRecipeTacos() {
        ...
        List<Category> categories = Flux.concat(
                categoryReactiveRepository.findByDescription("American"),
                categoryReactiveRepository.findByDescription("Mexican"))
                .collectList().block();
